...
- **EXAMPLE_QUESTIONS:** 
  ...
  - "Total revenue for last month" -> `{"period": "last_month"}`
- **JSON_FORMAT:** `{"tool": "get_sales_summary", "arguments": {"unit": "day", "quantity": 15}}`
...
- **EXAMPLES:** 
  ...
  - "Worst sellers of this week" -> `{"criterion": "least", "period": "this_week"}`
- **JSON_FORMAT:** `{"tool": "get_product_ranking", "arguments": {"limit": 5, "unit": "month", "quantity": 2}}`
...
- **DESCRIPTION:** Revenue percentage impact of an item.
- **DESCRIPTION:** Measures the percentage of total revenue generated by an item over a specific time range.
- **⚠️ WARNING:** Choose ONLY ONE mode: `period` OR (`unit` + `quantity`).
- **ARGUMENTS:** `{"product_identifier": string(Product name or SKU), "period": string, "unit": "day"|"week"|"month"|"year", "quantity": int}`
- **EXAMPLE_QUESTIONS:** 
...
- **IMPORTANT** ONLY USE THIS TOOL WHEN YOU ONLY NEED THE TOTAL NAMES OF THE PRODUCTS, FOR FILTERING BY SUPPLIRES OR PRICE, USE THE search_products_in_inventory TOOL, FOR MORE INFORMATION SEARCH IN THE SYSTEM CONTEXT
...
### 13 TOOL: get_all_product_names
* **TOOL_NAME:** `get_all_product_names`
* **DESCRIPTION:** Retrieves a simple list containing only the names of all products currently available in the inventory. Useful for quick catalog overviews without retrieving full product data, THIS TOOL ONLY RETRIVE THE NAME, THINGS LIKE SUPPLIER OR COST IS NOT FOR THIS TOOL, Use de search tool to filter products.
* **KEYWORDS:** product names, all items, inventory names, catalog list, what do we sell, product titles, stock names.
* **ARGUMENTS:** `{}`
* **EXAMPLE_QUESTIONS:** - "What are the names of all our products?" -> `{}`
  - "Give me a list of all the items we sell." -> `{}`
  - "List all product names currently in the inventory." -> `{}`
* **JSON_FORMAT:** `{"tool": "get_all_product_names", "arguments": {}}`

---

### 14 TOOL: get_all_promotions
...
### 15 TOOL: get_promotions_by_product
...
### 16 TOOL: get_all_customer_names
* **TOOL_NAME:** `get_all_customer_names`
* **DESCRIPTION:** Retrieves a simple list containing the unified full names (first and last name) of all registered customers. Useful for quick client overviews or populating dropdowns without fetching full customer profiles, DONT CALL THIS TOOL FOR FILTERING DATA, THERE IS A SERACH TOOL
FOR FILTERING CLIETS.
* **KEYWORDS:** customer names, list of clients, all customers, client full names, roster, directory, who are our customers.
* **ARGUMENTS:** `{}` *(This tool does not require any arguments)*
* **EXAMPLE_QUESTIONS:** - "What are the names of all our registered customers?" -> `{}`
  - "Give me a list of all clients." -> `{}`
  - "List the full names of everyone in our customer database." -> `{}`
* **JSON_FORMAT:** `{"tool": "get_all_customer_names", "arguments": {}}`

---

### 17 TOOL: get_total_customer_count
...
### 18 TOOL: search_customers (filtering)
...
### 19 TOOL: get_customer_points_history
...
### 20 TOOL: get_customer_credit_history
...
### 22 TOOL: get_total_supplier_count
...
### 23 TOOL: search_suppliers (filtering)
- **IMPORTANT:** THIS TOOL DONT RETRIEVE PRODUCTS INFORMATION ABOUT A SUPPLIER, USE THE SEARCH TOOL FOR PRODUCTS FILTER BY SUPPLIERS
...
### 24 TOOL: get_all_supplier_names
* **TOOL_NAME:** `get_all_supplier_names`
* **DESCRIPTION:** Retrieves a simple list containing only the names of all registered suppliers/vendors. Useful for quick directory overviews or populating selection lists without fetching full contact or tax details, THIS TOOL ITS NOT FOR FILTER SUPPLIER, USE DE SEARCH TOOL INSTEAD.
* **KEYWORDS:** supplier names, list of suppliers, all vendors, vendor list, who supplies us, supplier roster, distributors.
* **ARGUMENTS:** `{}` *(This tool does not require any arguments)*
* **EXAMPLE_QUESTIONS:** - "What are the names of all our suppliers?" -> `{}`
  - "Give me a list of every vendor we work with." -> `{}`
  - "List all supplier names in the database." -> `{}`
* **JSON_FORMAT:** `{"tool": "get_all_supplier_names", "arguments": {}}`

---

### 25 TOOL: get_all_chatbot_users
...
### 28 TOOL: system_capabilities
- **TOOL_NAME:** `system_capabilities`
- **DESCRIPTION:** Retrieves a comprehensive technical help guide that summarizes all available system modules (Sales, Inventory, CRM, Suppliers, and System Logic). Use this whenever the user asks for general help, "What can you do?", or for a summary of the system's functions.
- **KEYWORDS:** help menu, system capabilities, what can you do, guide, features, support, modules overview, assistance, how can you help me.
- **ARGUMENTS:** `{}` *(This tool does not require any arguments)*
- **EXAMPLE_QUESTIONS:** 
  - "What can you do?" -> `{}`
  - "Show me the help guide." -> `{}`
  - "What functions are available in this system?" -> `{}`
  - "In what areas can you assist me today?" -> `{}`
- **JSON_FORMAT:** `{"tool": "system_capabilities", "arguments": {}}`

---

### 30 TOOL: system_greeting_protocol
- **TOOL_NAME:** `system_greeting_protocol`
- **DESCRIPTION:** Use this tool whenever the user greets the system (e.g., "Hello", "Hi", "Good morning") or initiates social chat. This tool returns a professional enterprise greeting and immediately redirects the user to focus on POS system operations such as Sales, Inventory, Customers, or Suppliers.
- **KEYWORDS:** hello,get identity, hi, greetings, good morning, good afternoon, how are you, greetings, social chat, hola, saludos, buenos dias.
- **ARGUMENTS:** `{}` *(This tool does not require any arguments)*
- **EXAMPLE_QUESTIONS:** 
  - "Hello there!" -> `{}`
  ...
  - "Buenos dias" -> `{}`
  - "Who is this?" -> `{}`
- **JSON_FORMAT:** `{"tool": "system_greeting_protocol", "arguments": {}}`
